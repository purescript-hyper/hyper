...
import Hyper.Middleware (runMiddlewareT, MiddlewareT(MiddlewareT))
...
about =
  { path: ["about"]
  , "GET": handler (MiddlewareT $ \conn -> runMiddlewareT (html (linkTo contact (text "Contact Me!"))) conn)
  , "POST": notSupported
  }

contact =
  { path: ["contact"]
  , "GET": handler (MiddlewareT $ \conn -> runMiddlewareT (html (linkTo about (text "About Me"))) conn)
  , "POST": notSupported
  }
